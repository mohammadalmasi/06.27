...
SEMGREP_CONFIG = Path(__file__).resolve().parent / "semgrep_xss.yml"
...
def _run_semgrep(path: Path) -> list:
    ...
    env = os.environ.copy()
    log_path = Path(__file__).resolve().parent / ".semgrep.log"
...
def _run_static_scanner(path: Path, scanner: StaticXSSScanner) -> list:
    source = path.read_text(encoding="utf-8")
...
def main() -> None:
    ...
    # Run static XSS detector
    print("\nRunning static XSS detector...")
    static_scanner = StaticXSSScanner()
    ...
    # Summaries
    _summarize("Semgrep XSS", funcs, "semgrep_hit")
    _summarize("Static XSS detector", funcs, "static_hit")

    out_dir = Path(__file__).resolve().parent
    csv_path = out_dir / "comparison_semgrep_vs_static_xss.csv"
