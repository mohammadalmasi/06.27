...
#   5. If the number is >= 0.5 we say "potential XSS" and report the line.
# =============================================================================
...
# We use the same "vulnerability" object as the rule-based XSS scanner.
from scanners.xss.xss_scanner import XSSVulnerability

# Where files live
BACKEND_DIR = Path(__file__).resolve().parent.parent.parent
MODEL_DIR = BACKEND_DIR / "models"
DEFAULT_MODEL_PATH = MODEL_DIR / "bidirectional_LSTM_model_xss.h5"
...
# Word2Vec the BiLSTM was trained with (same as SQL: mincount=10, iterations=200, size=300).
...
def load_word2vec(w2v_path=None, embed_dim=None):
    ...
    if w2v_path and Path(w2v_path).is_file():
        try:
            ...
            pass
    # First: the exact model the BiLSTM was trained with.
...
class MLXSSDetector:
    ...
    def _load_model_and_w2v(self):
        ...
        if self._model is not None:
            return
        if self.verbose:
            print("[ML XSS] Loading model and Word2Vec...")
        if not HAS_KERAS:
            raise RuntimeError("ML XSS scanner needs TensorFlow. Run: pip install tensorflow")
        if not os.path.isfile(self.model_path):
            raise FileNotFoundError(
                f"Model not found: {self.model_path}. "
                "Put bidirectional_LSTM_model_xss.h5 in backend/models/."
            )
        ...
        if self.verbose:
            print(f"[ML XSS] Model loaded. Window length={self._window_length}, embed_dim={self._embed_dim}")
            if self._w2v is not None:
                print("[ML XSS] Word2Vec: loaded")
            else:
                if not HAS_GENSIM:
                    print("[ML XSS] Word2Vec: not found (install gensim: pip install gensim)")
                else:
                    print(f"[ML XSS] Word2Vec: not found (using zero vectors). Put {REQUIRED_WORD2VEC_FILENAME} in backend/models/")
    ...
    def scan_source(self, source_code, source_name="<source>"):
        ...
        if self.verbose:
            print(f"[ML XSS] Step 1: Read source — {len(lines)} lines")

        # Step 2: Tokenize
        tokens = tokenize_code(source_code)
        if not tokens:
            if self.verbose:
                print("[ML XSS] Step 2: Tokenize — no tokens, skipping")
            return []
        if self.verbose:
            print(f"[ML XSS] Step 2: Tokenize — {len(tokens)} tokens")
