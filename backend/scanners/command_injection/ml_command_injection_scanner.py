# =============================================================================
# ML COMMAND INJECTION SCANNER
# =============================================================================
# What this file does (in simple words):
#
#   1. Read a Python file.
#   2. Split the code into small words (tokens), e.g. "def", "os", ".", "system"
...
#   5. If the number is >= 0.5 we say "potential command injection" and report
#      the line.
# =============================================================================
...
# We use the same "vulnerability" object as the rule-based command injection scanner.
from scanners.command_injection.command_injection_scanner import CommandInjectionVulnerability

# Where files live
BACKEND_DIR = Path(__file__).resolve().parent.parent.parent
MODEL_DIR = BACKEND_DIR / "models"
DEFAULT_MODEL_PATH = MODEL_DIR / "Bidirectional_LSTM_model_command_injection.h5"
...
# Use this exact model for correct embeddings.
...
class MLCommandInjectionDetector:
    def __init__(
        self,
        model_path=None,
        w2v_path=None,
    ...
    def _load_model_and_w2v(self):
        ...
        if self._model is not None:
            return
        if self.verbose:
            print("[ML Command Injection] Loading model and Word2Vec...")
        if not HAS_KERAS:
            raise RuntimeError("ML Command Injection scanner needs TensorFlow. Run: pip install tensorflow")
        if not os.path.isfile(self.model_path):
            raise FileNotFoundError(
                f"Model not found: {self.model_path}. "
                "Put Bidirectional_LSTM_model_command_injection.h5 in backend/models/."
            )
        ...
        if self.verbose:
            print(f"[ML Command Injection] Model loaded. Window length={self._window_length}, embed_dim={self._embed_dim}")
            if self._w2v is not None:
                print("[ML Command Injection] Word2Vec: loaded")
            else:
                if not HAS_GENSIM:
                    print("[ML Command Injection] Word2Vec: not found (install gensim: pip install gensim)")
                else:
                    print(f"[ML Command Injection] Word2Vec: not found (using zero vectors). Put {REQUIRED_WORD2VEC_FILENAME} in backend/models/")
    ...
    def scan_source(self, source_code, source_name="<source>"):
        ...
        if self.verbose:
            print(f"[ML Command Injection] Step 1: Read source — {len(lines)} lines")

        tokens = tokenize_code(source_code)
        if not tokens:
            if self.verbose:
                print("[ML Command Injection] Step 2: Tokenize — no tokens, skipping")
            return []
        if self.verbose:
            print(f"[ML Command Injection] Step 2: Tokenize — {len(tokens)} tokens")
