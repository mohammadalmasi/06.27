...
SEMGREP_CONFIG = Path(__file__).resolve().parent / "semgrep_command_injection.yml"
...
def _run_semgrep(path: Path) -> list:
    ...
    env = os.environ.copy()
    log_path = Path(__file__).resolve().parent / ".semgrep_command.log"
...
def _run_static_scanner(path: Path, scanner: StaticCommandInjectionScanner) -> list:
    source = path.read_text(encoding="utf-8")
...
def main() -> None:
    ...
    # Summaries
    _summarize("Semgrep Command Injection", funcs, "semgrep_hit")
    _summarize("Static Command Injection detector", funcs, "static_hit")

    out_dir = Path(__file__).resolve().parent
    csv_path = out_dir / "comparison_semgrep_vs_static_command_injection.csv"
