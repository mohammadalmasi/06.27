...
This section provides direct answers to the research questions posed at the beginning of this thesis, synthesizing the methodologies, implementations, and evaluation results presented throughout the document.
...
This section provides a comprehensive comparison between Semgrep, a widely-used static analysis tool, and the custom web-based scanner developed in this thesis. The comparison covers their core approaches, capabilities, integration methods, and explains how the machine learning component is integrated into the custom scanner.

\subsection{How Semgrep Works}
\label{sec:semgrep_works}

Semgrep is a static analysis tool that matches rules over a program's Abstract Syntax Tree (AST). In addition to direct pattern matching, Semgrep can be configured for dataflow/taint analysis to track the flow of untrusted inputs to sensitive sinks. In practice, Semgrep is executed over a repository with a selected rule set (built-in, community, or custom YAML rules). Findings are reported with rule identifiers, file locations, and code snippets.

\subsubsection{Pattern Matching}
\label{sec:semgrep_patterns}

Semgrep patterns are structural: they match program constructs rather than raw text. Consequently, matching is robust to variations in whitespace, formatting, and identifier naming.
...
Semgrep rules follow a structured YAML format with the following components:
\begin{itemize}
...
In taint mode, Semgrep can follow data flow. It marks user input as unsafe (tainted), then checks if it reaches a dangerous place (like SQL execute). If tainted data reaches a sink without proper sanitization, Semgrep reports a vulnerability.
...
If a different pretrained model is adopted in the future (e.g., token-based embeddings rather than character one-hot encoding), the \texttt{input\_shape} will differ and preprocessing must be updated accordingly. The current implementation uses character-level encoding; token-based approaches (Word2Vec, BPE) could be integrated with appropriate preprocessing modifications.
