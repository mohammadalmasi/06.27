...
This chapter introduces the four vulnerability classes analyzed in this thesis: SQL Injection (SQLi), Cross-Site Scripting (XSS), Command Injection, and Cross-Site Request Forgery (CSRF). For each class, the discussion connects attack behavior to the detection logic used later in the static and machine learning components.

\section{SQL Injection}
\label{sec:background_sqli}

SQL Injection (SQLi) targets the interface between application code and the database engine~\cite{OWASPTop10,OWASPSQLiCheatsheet,CWE89,Halfond2005AMNESIA}. When user-controlled input is merged into query text, an attacker can alter query semantics and potentially:
\begin{itemize}
...
This SQLi variant appears when developers build query strings by directly concatenating untrusted input (for example, with \texttt{+} or equivalent string-join operations).

\textbf{How it works:} Because the database receives one final SQL string, any injected fragment is parsed as SQL syntax rather than plain data unless parameter binding is used.
...
Command Injection occurs when application code forwards untrusted input to shell-interpreted command execution~\cite{OWASPTop10,OWASPCommandInjection,CWE78}. In that case, an attacker may execute unintended operating-system commands and cause:
\begin{itemize}
...
This pattern refers to direct shell command execution through APIs such as \texttt{os.system()} and \texttt{os.popen()}.

\textbf{How it works:} If untrusted input is embedded in a shell command string, shell metacharacters (\texttt{;}, \texttt{\&\&}, \texttt{||}, \texttt{|}) can terminate or extend the intended command. The injected commands then run with the same privileges as the application process.
...
Cross-Site Scripting (XSS) occurs when untrusted data is rendered in a browser-executable context without adequate output encoding or sanitization~\cite{OWASPTop10,OWASPXSSCheatsheet,CWE79,MDNCSP,XSSRef1}. The result is that attacker-controlled script executes in the victim's browser session.

\subsection{Reflected XSS}
\label{sec:xss_reflected}

Reflected XSS (non-persistent XSS) appears when request input is echoed immediately in a response page (for example, search pages or error views) without safe encoding.

\textbf{How it works:} An attacker embeds script payloads in URL/query parameters or submitted form values. When that data is reflected into HTML unsafely, the browser executes the payload as page script.
...
Cross-Site Request Forgery (CSRF) abuses the browser's automatic credential handling to trigger unintended state-changing requests from authenticated users~\cite{OWASPTop10,OWASPCSRCheatsheet,OWASPSessionManagementCheatsheet,OWASPAuthenticationCheatsheet,CWE352,XSSRef2,MDNSameSite,RFC6265}. Instead of stealing credentials directly, the attacker causes the victim's browser to submit valid-looking requests to the target application, which can result in:
\begin{itemize}
...
This issue appears when POST endpoints process state-changing requests but do not verify an anti-CSRF token (or equivalent defense).

\textbf{How it works:} A malicious site can auto-submit a form or script to the vulnerable endpoint. If the victim is logged in, browser-managed cookies are attached automatically, so the server may accept the request as authentic.
